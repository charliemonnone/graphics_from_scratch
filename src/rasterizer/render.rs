use super::{
    ...
    data_types::Instance,
    data_types::Point,
    data_types::{Mat4x4, Model, Triangle, Vertex4},
    clipping::transform_and_clip,
    main::get_canvas_dimensions,
    utils,
};
...
pub fn render_scene(image: &mut Image, cam: &Camera, instances: &Vec<Instance>) {
    ...
    for i in instances {
        ...
        if let Some(clipped) = transform_and_clip(&cam.clipping_planes, i.model, transform) {
            render_model(image, clipped, transform);

        } 
    }
}

pub fn render_model(image: &mut Image, model: Model, transform: Mat4x4) {
    let mut projected: Vec<Point> = vec![];

    for v in &model.verticies {
        let v = Vertex4::new(v.x, v.y, v.z, 1.);
        projected.push(utils::project_vertex(v));
    }
...
fn put_pixel(image: &mut Image, x: i32, y: i32, color: Color) {
    ...
    if x_mapped >= width as u32 || y_mapped >= height as u32 {
        return;
    }
